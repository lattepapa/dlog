...
이를 위해 각 제수들(2, 3, 5)들로 입력값을 나눠서, 입력값이 1이 될때까지 만드는 작업이 필요하다. 참고로 나눗셈 작업은 $O(log{_{제수}}{피제수})$의 시간복잡도를 갖는다. 여기선 2로 나눌 때의 시간복잡도가 가장 크기 때문에 **isUgly** 함수는 $O(log{_2}{n})$의 시간복잡도를 갖는다. 이러한 나눗셈 작업은 중첩된 반복문에서 실행되므로 최종 시간복잡도는 $O(nlog{_2}{n})$이 된다.
